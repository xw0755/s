  ...
  goog.writeScripts_ = function() {
    ...
    for (var i = 0; i < scripts.length; i++) {
      if (scripts[i]) {
		...
		while (true) {
			pos = scriptSrc.indexOf(str);
			if (pos == -1||pos>0) break;

			counter++;
			scriptSrc = scriptSrc.substring(str.length);
		}
		if(counter>0)
		{
			pos = sbasePath.lastIndexOf("/");
			if (pos == (sbasePath.length - 1))
				sbasePath = sbasePath.substring(0,pos);
		...
		//alert(goog.basePath+scripts[i]+"/n"+sbasePath + scriptSrc);
        goog.importScript_(sbasePath + scriptSrc);
      } else {
